<?php
/**
 * BONIFACIOS SYNC - VERSION SERVIDOR CON LOGGING COMPLETO
 * Copia este archivo a C:\Sincronizador\softrestaurant-sync\
 * y corre: php C:\Sincronizador\softrestaurant-sync\sync-server.php
 */
...
class SyncServer {
    ...
    public function __construct() {
        echo "=== Bonifacio's Sync Servidor ===\n";
    }
    ...
    private function syncToday(): void {
        try {
            ...
            echo "[HOY] Cerrados encontrados: " . count($closed) . "\n";
            ...
            echo "[HOY] Abiertos encontrados: " . count($open) . "\n";

            $rows = array_merge($closed, $open);

            if (count($rows) === 0) {
                echo "[HOY] Sin ventas en este turno.\n";
                return;
            }
            ...
            $upd  = $result['updated']  ?? '?';
            $fail = $result['failed']   ?? '?';
            echo "[RESULTADO] inserted=$ins | updated=$upd | failed=$fail\n";

            if (!empty($result['error'])) {
                echo "[API ERROR] " . $result['error'] . "\n";
            }
            if (($result['failed'] ?? 0) > 0) {
                echo "!!! HAY FALLOS - revisa los error_log de PHP en Hostinger\n";
            }

        } catch (Throwable $e) {
            echo "[ERROR syncToday] " . $e->getMessage() . "\n";
        }
    }
