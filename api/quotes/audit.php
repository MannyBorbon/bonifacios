...
try {
    ...
    $isQuotesAdmin = in_array($role, ['administrador', 'admin', 'superadmin'], true);
    ...
    if ($method === 'GET') {
        ...
        if ($action === 'history' && $quoteId > 0) {
            $stmt = $conn->prepare(
                'SELECT qal.*, u.full_name AS user_full_name, u.username '
                . 'FROM quote_audit_log qal LEFT JOIN users u ON qal.user_id = u.id '
                . 'WHERE qal.quote_id = ? ORDER BY qal.created_at DESC LIMIT 100'
            );
            ...
            $stmt = $conn->prepare(
                'SELECT qv.*, u.full_name AS restored_by_name FROM quote_versions qv '
                . 'LEFT JOIN users u ON qv.created_by = u.id WHERE qv.quote_id = ? ORDER BY qv.version_number DESC'
            );
            $stmt->bind_param('i', $quoteId);
            $stmt->execute();
            ...
            $stmt = $conn->prepare(
                'SELECT * FROM quote_versions WHERE quote_id = ? AND version_number = ?'
            );
            ...
            if ($version) {
                ...
                echo json_encode(['success' => false, 'error' => 'Versión no encontrada']);
            }
        } elseif ($action === 'summary') {
            $stmt = $conn->prepare(
                'SELECT * FROM quote_version_summary ORDER BY last_version_date DESC'
            );
            $stmt->execute();
            ...
            echo json_encode(['success' => false, 'error' => 'Acción no válida o falta quote_id']);
        }
    } elseif ($method === 'POST') {
        if (!$isQuotesAdmin) {
        ...
        if ($action === 'log') {
            ...
            if ($quoteId === 0) {
                http_response_code(400);
                echo json_encode(['success' => false, 'error' => 'quote_id requerido']);
                exit;
            }
            ...
            if ($quoteId === 0 || $versionNumber === 0) {
                http_response_code(400);
                echo json_encode(['success' => false, 'error' => 'quote_id y version_number requeridos']);
                exit;
            }
            ...
            if ($stmt->execute()) {
                ...
                echo json_encode(['success' => false, 'error' => 'Error al restaurar versión: ' . $stmt->error]);
            }
            ...
            if ($quoteId === 0 || $version1 === 0 || $version2 === 0) {
                http_response_code(400);
                echo json_encode(['success' => false, 'error' => 'quote_id, version_1 y version_2 requeridos']);
                exit;
            }

            $stmt = $conn->prepare(
                'SELECT * FROM quote_versions WHERE quote_id = ? AND version_number IN (?, ?) ORDER BY version_number'
            );
            ...
            if (count($versions) !== 2) {
                http_response_code(404);
                echo json_encode(['success' => false, 'error' => 'Una o ambas versiones no encontradas']);
                exit;
            }

            $differences = [];
            $fields = [
                'name', 'phone', 'email', 'event_type', 'event_date', 'guests',
